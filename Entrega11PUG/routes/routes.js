...
// Rutas con interfaz grafica PUG
router.get('/productos/vista', async (req, res) => {
  const result = await productos.leer();
  res.render('index', { products: result, listExists: result.length > 0 ? true : false })
})

router.get('/productos/guardar', async (req, res) => {
  res.render('addProduct', { error: false });
})
...
router.post('/api/productos/guardar/', async (req, res) => {
  ...
  if(result.error){
    res.render('addProduct', { error: result.msg });
  } else {
